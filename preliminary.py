...
def labor_costs(annual_work_hour):
    '''
    This function get hourly labor costs from annual wages. Hourly labor cost is calculated by using annual wages, befefit rate, and paid vacation hours. (ALCTS Technical Services Costs Committee. "Guide to Cost Analysis of Acquisitions and Cataloging in Libraries," ALCTS Newsletter 2. no. 5, 1991: 49-52.)
...
def cataloging_cost(annual_work_hour):
    '''
    This function calculates cataloging cost per a book. The function randomly choose a catalog's productivity per day. (Robert H. Burger, Financial Management of Libraries and Information Centers, California and Colorado: Libraries Unlimited, 2017, 249-253.)
    :param annual_work_hour: integer
    :return: float.
...
def get_book_list(num_of_titles, annual_work_hour):
    ...
    :param num_of_titles: Integer. a number of book titles published or considered for purchasing.
    :return: DataFrame, columns are thickness and price, demands, and cataloging cost per a book.
...
def select_book(plan, budget, space):
    '''
    This function selects books from the book list generated by get book list function. The function selects books base on budget and space and then drop nan.
    :param plan: Data Frame
    :param budget: integer. A Library's annual budget for acquisitions including cataloging and maintenance consts.
...
def MonteCarloSimulation(annual_work_hour, total_volume, budget, space, num_of_titles) -> list:
    ...
    :param annual_work_hour: integer. totla annual work hour.
    :param total_volume: integer. A library's current volumes of collection.
    :param budget: float. annual budget for acquisitions, maintenance, and cataloging labor costs.
    :param space: float. available space to shelve.
    :param num_of_titles: integer. a number of book titles published or considered for purchasing.
    :return: list which contains of results, number of books, costs, and thickness from the both strageties.
